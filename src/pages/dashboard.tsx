      ...
      <section className="mt-12">
        ...
        {!isConnected ? (
          ...
          <div className="grid gap-4 md:grid-cols-3">
            {[0, 1, 2].map((item) => (
              <div key={item} className="h-64 animate-pulse rounded-2xl border border-white/[.06] bg-white/[.025]" />
            ))}
          </div>
        ) : campaigns.length ? (
  ...
  return (
    <div className="grid min-h-64 place-items-center rounded-2xl border border-dashed border-white/[.09] bg-white/[.012] p-8 text-center">
      <div>
        ...
        {action && (
          <Link to="/app/campaigns/new" className="mt-5 inline-block text-sm font-semibold text-mint">
            Create campaign
          </Link>
        )}
      </div>
