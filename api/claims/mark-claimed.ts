...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  const parsed = parseMarkClaimedBody(req.body);
  if (!parsed) return badRequest(req, res);

  const expectedMessage = `Privlo mark claim claimed ${parsed.claimId} for ${parsed.recipient}`;
  if (parsed.message !== expectedMessage) return unauthorized(req, res);

  const verified = await verifyWalletMessage({
    expectedAddress: parsed.recipient,
  ...
  try {
    const updated = await markClaimClaimed({
      recipient: parsed.recipient,
      claimId: parsed.claimId,
      transactionHash: parsed.transactionHash,
    });
    if (!updated) return badRequest(req, res);
